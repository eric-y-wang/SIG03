---
title: "SIG03 T7GEX 6RT Mouse 10x Seurat Processing"
author: "Eric Y. Wang"
date: "`r Sys.Date()`"
output:
  html_notebook:
...
```{r}
dataRaw <- Read10X_h5("C:/Users/Eric/My Drive/Lab/datasets/EYW/SIG03_10x_240706/cellranger_outs/SIG03_GEX_HTO_T7GEX_6RT_mouse/filtered_feature_bc_matrix.h5")
names(dataRaw)
```
...
dataCD4$seurat_clusters <- dataCD4$SCT_snn_res.0.8
...
Cluster 7,15 seem to be contaminating B cells.

```{r}
dataCD4Sub <- subset(dataCD4, invert = TRUE, subset = seurat_clusters %in% c("7","15"))
...
```{r}
dataSplenocytes$seurat_clusters <- dataSplenocytes$SCT_snn_res.1
...
metadata <- dataSplenocytes@meta.data %>%
  as_tibble() %>%
  mutate(clusters_anno = case_when(seurat_clusters %in% c("0","1","2","3","6") ~ "Bcells",
                                   seurat_clusters %in% c("4","8") ~ "Tcell",
                                   seurat_clusters %in% c("5") ~ "5",
                                   seurat_clusters == "7" ~ "NKcell",
                                   seurat_clusters == "9" ~ "Macrophage",
                                   seurat_clusters == "10" ~ "Neutrophil"))
...
```{r}
saveRDS(dataCD4Sub, "C:/Users/Eric/My Drive/Lab/datasets/EYW/SIG03_10x_240706/seurat_outs/SIG03_mouse_T7GEX6RT_CD4.rds")
saveRDS(dataSplenocytes, "C:/Users/Eric/My Drive/Lab/datasets/EYW/SIG03_10x_240706/seurat_outs/SIG03_mouse_T7GEX6RT_splenocytes.rds")
```
